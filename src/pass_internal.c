...
#ifndef WYPE_IO_BLOCKSIZE
#define WYPE_IO_BLOCKSIZE ( 4 * 1024 * 1024UL ) /* 4 MiB I/O block */
#endif

#if WYPE_IO_BLOCKSIZE > INT_MAX
#error "WYPE_IO_BLOCKSIZE must fit in an int"
#endif

/*
 * wype_(p)write_with_retry / wype_(p)read_with_retry
 *
...
#ifndef WYPE_MAX_IO_ATTEMPTS
#define WYPE_MAX_IO_ATTEMPTS 3
#endif

#ifndef WYPE_IO_RETRY_DELAY_S
#define WYPE_IO_RETRY_DELAY_S 5
#endif

#if WYPE_MAX_IO_ATTEMPTS < 1
#error "WYPE_MAX_IO_ATTEMPTS must be at least 1"
#endif
...
ssize_t wype_write_with_retry( wype_context_t* c, int fd, const void* buf, size_t count )
{
    ...
    for( attempt = 0; attempt < WYPE_MAX_IO_ATTEMPTS; attempt++ )
    {
        ...
        if( attempt + 1 < WYPE_MAX_IO_ATTEMPTS )
        {
            c->io_retries += 1;
            c->retry_status = 1;

            wype_log( WYPE_LOG_NOTICE, "%s: retrying in %d seconds ...", __FUNCTION__, WYPE_IO_RETRY_DELAY_S );

            for( slept_s = 0; slept_s < WYPE_IO_RETRY_DELAY_S; slept_s++ )
            {
                sleep( 1 );
                pthread_testcancel();
            }
            ...
            if( r > 0 )
            {
                if( lseek( fd, -r, SEEK_CUR ) == (off64_t) -1 )
                {
                    ...
                    wype_log(
                        WYPE_LOG_ERROR, "%s: cannot rewind after short write on '%s'.", __FUNCTION__, c->device_name );
                    c->retry_status = 0;
        ...
        if( attempt + 1 < WYPE_MAX_IO_ATTEMPTS )
        {
            ...
            if( r > 0 )
            {
                if( lseek( fd, -r, SEEK_CUR ) == (off64_t) -1 )
                {
                    wype_perror( errno, __FUNCTION__, "lseek" );
                    wype_log(
                        WYPE_LOG_ERROR, "%s: cannot rewind after short read on '%s'.", __FUNCTION__, c->device_name );
                    c->retry_status = 0;
                    return -1; /* fatal, we don't know where we are */
                }
            }
...
int wype_fdatasync( wype_context_t* c, const char* f )
{
    ...
    if( r != 0 )
    {
        ...
        wype_perror( errno, f, "fdatasync" );
        wype_log( WYPE_LOG_WARNING, "Buffer flush failure on '%s'.", c->device_name );
 ...
 * - Starts from WYPE_IO_BLOCKSIZE (4 MiB by default) and adjusts.
...
size_t wype_effective_io_blocksize( const wype_context_t* c )
{
    ...
    size_t io_bs = (size_t) WYPE_IO_BLOCKSIZE;
...
int wype_compute_sync_rate_for_device( const wype_context_t* c, size_t io_blocksize )
{
    ...
    return (int) tmp;
} /* wype_compute_sync_rate_for_device */
...
{
    if( synced || c->io_mode == WYPE_IO_MODE_DIRECT )
    {
