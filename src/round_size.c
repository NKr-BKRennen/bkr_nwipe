                                           ...
                                           uint64_t* effective_pass_size_out )
{
    ...
    if( verify == WYPE_ROUND_VERIFY_ALL )
    {
        pass_size *= 2ULL;
    }
    ...
    if( noblank == 0 )
    {
        ...
        if( verify == WYPE_ROUND_VERIFY_LAST || verify == WYPE_ROUND_VERIFY_ALL )
        {
            round_size += device_size;
        }
    }
    else if( verify == WYPE_ROUND_VERIFY_LAST )
    {
        round_size += device_size;
    }

    if( method_class == WYPE_ROUND_METHOD_OPS2 )
    {
        round_size += device_size;
    ...
    else if( method_class == WYPE_ROUND_METHOD_IS5ENH )
    {
        if( verify == WYPE_ROUND_VERIFY_LAST && noblank == 1 )
        {
            round_size -= device_size;
        }
