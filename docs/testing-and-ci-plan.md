...
- `.github/workflows/ci_runtime.yml`
  - `unit-tests`: builds via Autoconf/Automake, runs deterministic unit tests, then a CLI smoke run
  - `integration-loopback`: full loopback integration suite
  - `sanitizer-smoke`: Autoconf build with ASan+UBSan + short smoke suite

- `.github/workflows/ci_nightly_faults.yml`
  - nightly + manual Autoconf fault-injection run
